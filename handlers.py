...
def handle_peticion_telefono(texto_usuario):
    ...
    if database.tiene_cita_futura(telefono, negocio_id=_get_negocio_id()):
        session.clear()
        return { "respuesta": "¡Ojo! Ya tienes una cita pendiente. Si quieres gestionarla, empieza de nuevo y elige 'Gestionar Cita'.", "nuevo_estado": None }
    ...
    lista_servicios_str = "".join([f"\n› {s['nombre']} — {s['precio']}€" for s in servicios_db])
    respuesta_completa = f"{mensaje_inicial}{lista_servicios_str}{pregunta_final}"
    
    return {"respuesta": respuesta_completa, "nuevo_estado": "pidiendo_servicio"}
...
def handle_peticion_servicio(texto_usuario):
    ...
    if not servicio_elegido: 
        return {"respuesta": "No he entendido. Elige uno de los servicios.", "nuevo_estado": "pidiendo_servicio"}
    session['servicio'] = servicio_elegido
    
    empleados = database.listar_empleados(negocio_id=_get_negocio_id())
    if not empleados: 
        session['empleado_id'] = None
        session['empleado_nombre'] = None # Asegurarnos de que no hay empleado fantasma
        return _mostrar_calendario()
    
    session['empleados_disponibles'] = {e['nombre']: e['id'] for e in empleados}
...
def _mostrar_horas_para_fecha(fecha_str):
    try:
        ...
        nuevo_estado = 'modificar_confirmar_hora' if session.get('modificando_cita') else 'confirmar_cita'
...
def handle_confirmar_cita(texto_usuario):
    ...
    if not (':' in hora_elegida and len(hora_elegida) == 5): 
        return {"respuesta": "Por favor, pulsa uno de los botones de hora.", "nuevo_estado": "confirmar_cita"}
    ...
    fecha_legible = datetime.strptime(session.get('fecha'), '%Y-%m-%d').strftime('%d/%m/%Y')
    ...
    session.clear()
    return {"respuesta": respuesta_final, "nuevo_estado": None}
...
def handle_gestion_pide_telefono(texto_usuario):
    ...
    fecha_legible = cita['fecha'].strftime('%A, %d de %B')
    hora_legible = cita['hora'].strftime('%H:%M')
...
def handle_gestion_confirmar_cancelacion(texto_usuario):
    ...
    if 'si' in respuesta_norm:
        ...
        if cita_a_cancelar: 
            database.cancelar_cita(cita_a_cancelar['id'], negocio_id=_get_negocio_id())
        session.clear()
        return {"respuesta": "¡Hecho! Tu cita ha sido cancelada.", "nuevo_estado": None}
    elif 'no' in respuesta_norm: 
        session.clear()
        return {"respuesta": "De acuerdo. No he cancelado tu cita.", "nuevo_estado": None}
    else: 
...
def handle_gestion_pide_campo_a_modificar(texto_usuario):
    ...
    if 'servicio' in texto_norm:
        ...
        session['nombres_servicios_disponibles'] = nombres_servicios
        lista_servicios_str = "".join([f"\n› {s['nombre']} — {s['precio']}€" for s in servicios_db])
        return {"respuesta": f"Entendido. ¿Por cuál de estos servicios quieres cambiarla?{lista_servicios_str}", "nuevo_estado": "pidiendo_servicio"}
...
def handle_modificar_servicio(texto_usuario):
    ...
    session.clear()
    return {"respuesta": f"¡Listo! He cambiado el servicio de tu cita a **'{nuevo_servicio}'**. El día y la hora se mantienen.", "nuevo_estado": None}
...
def handle_modificar_confirmar_hora(texto_usuario):
    ...
    return {"respuesta": f"¡Cita actualizada! Tu nueva cita es el **{fecha_legible} a las {hora_elegida}**.", "nuevo_estado": None}